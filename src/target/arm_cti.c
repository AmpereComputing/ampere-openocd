...
{
	struct adi_ap *ap = dap_ap(self->spot.dap, self->spot.ap_num);
	uint32_t tmp;
...
COMMAND_HANDLER(handle_cti_dump)
{
	...
	struct adi_ap *ap = dap_ap(cti->spot.dap, cti->spot.ap_num);
	int retval = ERROR_OK;
...
static int cti_configure(Jim_GetOptInfo *goi, struct arm_cti *cti)
{
	...
	while (goi->argc > 0) {
		int e = adi_jim_mem_ap_spot_configure(&cti->spot, goi);
		if (e != JIM_OK)
			return e;
	}
