...
static struct adiv5_ap *_dap_get_ap(struct adiv5_dap *dap, uint64_t ap_num, bool is_config)
{
	...
	if (is_adiv6(dap)) {
		for (unsigned int i = 0; i <= DP_APSEL_MAX; i++) {
			...
			if (ap->refcount && ap->ap_num == ap_num) {
				ap->refcount += 2;
				return ap;
			}
		}
...
{
	struct adiv5_ap *ap = _dap_get_ap(dap, ap_num, false);
	if (ap)
...
{
	struct adiv5_ap *ap = _dap_get_ap(dap, ap_num, true);
	if (ap)
...
int adiv6_dap_read_baseptr(struct command_invocation *cmd, struct adiv5_dap *dap, uint64_t *baseptr)
{
	...
	if (dap->asize > 32) {
		retval = dap_dp_read_atomic(dap, DP_BASEPTR1, &baseptr_upper);
		if (retval != ERROR_OK)
			return retval;
	}
				...
				struct adiv5_ap *ap, target_addr_t dbgbase, int depth)
{
	...
	if (class == ARM_CS_CLASS_0X1_ROM_TABLE) {
		...
		command_print(cmd, "\t\tDev Arch is 0x%08" PRIx32 ", %s \"%s\" r.%u", devarch,
